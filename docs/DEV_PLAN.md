...
| #63, #84 | Development feedback vs sealed OOS evidence | Prevents final out-of-sample results from becoming optimization inputs for later factor generations. | Add typed feedback provenance, allow only bounded development diagnostics into refinement, and record OOS results as terminal evidence. |
...
| #63, #84 | Development feedback vs sealed OOS evidence | P0 | done | Codex task 019f8ea8 | 2026-07-23 19:09 +0800 | 2026-07-23 20:09 +0800 | Implemented typed provenance and per-source prompt allowlists, categorical dev diagnostics, terminal OOS evidence/failure reasons, report audit state, and leakage regression tests. Verified: `pytest tests/test_align.py tests/test_backtest.py tests/test_agent_parsing.py tests/test_agents.py tests/test_factors.py -q` (41 passed); `pytest -q` (71 passed); `python -m compileall -q src tests`; `git diff --check`. |
...
| #56, #93, #95, #94 | Walk-forward window health summary | P1 | done | Codex orchestrator / subagent-walk-forward | 2026-07-24 00:52 +0800 | 2026-07-24 01:52 +0800 | Added per-window health status/flags, aggregate pass rate/risk flags, and factor-card rendering. #93/#95 duplicate #56; #94 was mojibake for the same request. Closed #93/#94/#95 upstream as duplicates on 2026-07-24. Verified: `pytest tests/test_backtest.py tests/test_agents.py -q` (29 passed); `pytest tests/test_audit.py tests/test_backtest.py tests/test_agents.py -q` (39 passed). |
...
| #105 | Track research panel source in git | P1 | done | Codex issue triage | 2026-07-24 09:40 +0800 | 2026-07-24 10:40 +0800 | Verified `panel/` is tracked with 40 source/config files, including `panel/public/data/manifest.json`; no `panel/node_modules/`, `panel/dist/`, or generated panel data files are tracked. Commented upstream with local verification and left open until the ahead local branch is pushed. Verified: `git ls-files panel | wc -l`; `git ls-files panel`; `cd panel && pnpm build`. |
